...
jugadores, pero no indica cuántas celdas ocupa cada uno. Agregar una indicación visual del
tamaño (número de celdas) en cada ítem del panel de flota, tanto en "Tu Flota" como en
"Flota Enemiga", usando mini-bloques visuales similares a los que ya existen en la fase de
colocación.
...
2. Modificar `renderFleetPanel` para que, al construir cada `<li>`, añada mini-bloques
   visuales (`fleet-item-block`) que representen el tamaño del barco, en lugar de (o junto
   a) solo el nombre.
...
- `js/game.js` — Contiene el array `SHIPS` (línea 31-37) y la función `renderFleetPanel`
  (línea 150-161). Aquí se agrega `size` a `SHIPS` y se modifica el renderizado del panel.
- `css/styles.css` — Contiene los estilos del panel de flota (`.fleet-item`, `.fleet-list`,
  etc., líneas 354-412). Aquí se agregan estilos para los mini-bloques de tamaño.

## Implementation Plan

### Phase 1: Foundation — Datos de tamaño
Agregar la propiedad `size` a cada objeto del array `SHIPS` en `game.js`, asegurando que
todos los componentes que itereran sobre `SHIPS` tengan acceso al tamaño del barco.
...
- Verificar que ningún otro código existente sea afectado (el array solo se usa en
  `renderFleetPanel` y en la notificación de hundimiento)

### 2) Modificar `renderFleetPanel` en `js/game.js` para mostrar mini-bloques
- En la función `renderFleetPanel` (línea ~150), reemplazar `li.textContent = ship.name`
  por construcción manual de elementos hijos:
...
- Verificar que los mini-bloques reflejan el tamaño correcto (5 para Portaaviones, 2 para
  Destructor, etc.)
- Verificar que al hundir un barco, los mini-bloques cambian de color junto al nombre
  tachado
...
6. Hundir el Destructor (2 celdas) y verificar que sus 2 mini-bloques cambian a rojo
...
- **Barcos hundidos**: Los mini-bloques deben adoptar el color de hundido junto con el
  texto tachado (ya manejado por `.fleet-item--sunk .fleet-item-block`)
- **Pantallas pequeñas (móvil)**: El panel de flota cambia a `flex-direction: column` en
  mobile (línea ~1011 del CSS). Los mini-bloques de 8px son suficientemente pequeños para
  no desbordar en pantallas estrechas
...
- **Re-renders de Firebase**: `renderFleetPanel` limpia `innerHTML` y reconstruye desde
  cero en cada llamada, por lo que los mini-bloques se reconstruyen correctamente
...
- Los mini-bloques (`fleet-item-block`) reutilizan el lenguaje visual de `.ship-cell-block`
  de la fase de colocación, pero con tamaño reducido (8px × 8px vs los bloques de la
  fase de colocación que son más grandes) para adaptarse al panel compacto.
- Se usa `margin-left: auto` en `.fleet-item-blocks` para alinear los bloques a la derecha
  del ítem, dejando el nombre a la izquierda. Esto aprovecha el `display: flex` ya
  existente en `.fleet-item`.
