...
Expected: Ambos jugadores regresan a la fase de colocación de barcos en la misma sala

## Root Cause
En `js/game.js` (función `handleGameFinished`, líneas ~422-432), el handler de `btn-rematch` llama a `window.location.reload()`, idéntico al handler de `btn-exit`:
...
## Solution Approach
1. Agregar `resetRoom(roomId)` en `firebase-game.js` que escribe el estado de sala reseteado en Firebase
...
- `handleGameFinished` en `game.js` (~líneas 362-433)
- `listenRoom` en `firebase-game.js` (~líneas 53-91)
...
- Verificar que `update` y `ref` ya están importados (sí, están en la línea 4)
...
- Al inicio del callback `onValue`, antes de los checks existentes, agregar la detección de revancha:
  ...
  ```
- Esto asegura que `onTurnChange`, `onAttacksChange` y `onGameFinished` funcionen correctamente en la segunda partida
...
- `{ once: true }` en los handlers de `btnRematch` y `btnExit` previene que se acumulen listeners adicionales si `handleGameFinished` se llama múltiples veces (ej. en el caso extremo de bugs de Firebase que disparen el callback dos veces)
